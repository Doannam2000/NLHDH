...
void Gantt(Node a[], int n)
{
	...
	int d = k / 10; // so dong
	if (k % 10 != 0) // neu k%10 du thi tang them so dong vd 25 => so dong = 2 ++ = 3;
		d++;
	a[0].TGC = a[0].Txh;
	while (i < d)
	{
...
void ThoiGianCho(Node a[], int n)
{
	...
	cout << "\tThoi gian cho trung binh cua he :" << Tb/n << endl;
}
void ThoiGianXuLy(Node a[], int n)
{
	...
	cout << "\tThoi gian xu ly trung binh cua he :" << Tb / n << endl;
}
int main()
{
	int n;
	...
	for (int i = 0; i < n; i++)
	{
		a[i].P = "P"+to_string(i); //to_string dung trong c++11
		do
		{
